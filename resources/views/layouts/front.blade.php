...
<head>
    ...
    {{-- ✅ CSRF token (utile pour AJAX / formulaires) --}}
    ...
    <style>
        ...
        .aa-btn { background: linear-gradient(90deg, #F9E4B7, #D4AF37); color:#1C1C1C; }
        ...
        /* Bonus si "nav-link" n'est pas défini dans app.css */
        .nav-link{ color:#111827; font-weight:600; opacity:.85; transition:.2s; }
        .nav-link:hover{ color:#b58900; opacity:1; }
    </style>
...
@php
    $authUser = auth()->user();
    // ✅ idéalement fourni par View::composer (AppServiceProvider). Fallback si absent.
    $headerNonLusCount = $headerNonLusCount ?? 0;
@endphp
            ...
            <div class="flex items-center justify-between gap-4 py-2">
                ...
                <a href="{{ route('front.index') }}" class="flex items-center gap-2 group">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"
                         class="h-9 w-9 -mt-0.5 transition-transform duration-300 group-hover:scale-110">
                        <defs>
                    ...
                    <div class="leading-tight font-extrabold -mt-0.5">
                        <div class="text-[#b58900] text-base tracking-tight">LLO</div>
                        <div class="text-[#b58900] text-base -mt-1">PPART</div>
                    </div>
                </a>

                {{-- Barre de recherche (desktop) — plus compacte --}}
                <form action="{{ route('front.index') }}" method="GET"
                      class="hidden md:flex items-center gap-3 px-4 py-2 rounded-full
                             border border-[#facc15]/35 bg-white/70 backdrop-blur-xl shadow-sm
                             hover:shadow-md transition-all duration-300">

                    <div class="flex items-center gap-2 pl-1">
                        <i class="fa-solid fa-location-dot text-[#b58900] text-sm opacity-80"></i>
                        <input name="q" value="{{ request('q') }}"
                               class="bg-transparent text-gray-700 placeholder-gray-400 focus:outline-none text-sm w-52"
                               type="text" placeholder="Localité / type…">
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input name="min" value="{{ request('min') }}"
                               class="bg-transparent focus:outline-none text-sm w-20 text-gray-700 placeholder-gray-400"
                    ...
                    <button type="submit"
                            class="ml-1 flex items-center gap-2 px-3 py-1.5 rounded-full
                                   bg-[#facc15] text-gray-900 font-semibold shadow-sm
                                   hover:bg-[#fddb4f] hover:shadow hover:scale-[1.02]
                                   transition-all duration-300 focus:ring-2 focus:ring-[#facc15]/40">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-[16px] w-[16px]" viewBox="0 0 24 24" fill="none" stroke="#1C1C1C" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="hidden md:flex items-center gap-4">
                    ...
                    @auth
                        @php
                            $u = $authUser;
                            $nonLus = (int) ($headerNonLusCount ?? 0);
                            $initials = mb_strtoupper(
                                mb_substr(($u->prenom ?? ''), 0, 1) . mb_substr(($u->nom ?? 'U'), 0, 1)
                            );
                        @endphp

                        {{-- ✅ Un seul bouton Profil (on met Messages/Favoris DEDANS) --}}
                        <div class="relative" id="profileDropdownWrap">
                            <button type="button" id="profileDropdownBtn"
                                    class="inline-flex items-center gap-3 rounded-full border aa-border bg-white/70 px-3 py-1.5 hover:bg-white transition">
                                <span class="h-9 w-9 rounded-full bg-[#fff5d6] border border-[#facc15]/40 grid place-items-center text-[#b58900] font-extrabold text-sm">
                                    {{ $initials }}
                                </span>
                                <span class="text-sm font-semibold text-gray-900 max-w-[140px] truncate">
                                    {{ trim(($u->prenom ?? '').' '.($u->nom ?? '')) ?: ($u->nom ?? 'Mon compte') }}
                                </span>
                                <i class="fa-solid fa-chevron-down text-gray-500 text-xs"></i>
                            </button>

                            <div id="profileDropdownMenu"
                                 class="hidden absolute right-0 mt-2 w-72 overflow-hidden rounded-2xl border aa-border bg-white shadow-xl">
                                <div class="px-4 py-3 border-b aa-border">
                                ...
                                {{-- Raccourcis : Favoris + Messages (badge) --}}
                                <div class="p-3 border-b aa-border">
                                    <div class="grid grid-cols-2 gap-2">
                                        @if(\Illuminate\Support\Facades\Route::has('favoris.index'))
                                            <a href="{{ route('favoris.index') }}"
                                               class="flex items-center justify-center gap-2 rounded-xl border aa-border px-3 py-2 hover:bg-gray-50 text-sm font-semibold">
                                                <i class="fa-solid fa-heart text-[#b58900]"></i>
                                                Favoris
                                            </a>
                                        @else
                                            <span class="flex items-center justify-center gap-2 rounded-xl border aa-border px-3 py-2 text-sm text-gray-400">
                                                <i class="fa-solid fa-heart"></i> Favoris
                                            </span>
                                        @endif

                                        @if(\Illuminate\Support\Facades\Route::has('client.messages'))
                                            <a href="{{ route('client.messages') }}"
                                               class="relative flex items-center justify-center gap-2 rounded-xl border aa-border px-3 py-2 hover:bg-gray-50 text-sm font-semibold">
                                                <i class="fa-solid fa-comments text-[#b58900]"></i>
                                                Messages
                                                @if($nonLus > 0)
                                                    <span class="absolute -top-2 -right-2 h-6 min-w-[24px] px-2 grid place-items-center rounded-full bg-[#facc15] text-[#1C1C1C] text-[11px] font-extrabold shadow">
                                                        {{ $nonLus }}
                                                    </span>
                                                @endif
                                            </a>
                                ...
                                <div class="p-2 grid gap-1">
                                    <a href="{{ route('dashboard') }}"
                                       class="px-3 py-2 rounded-xl hover:bg-gray-50 text-sm flex items-center gap-2">
                                        <i class="fa-solid fa-gauge-high text-[#b58900] w-4"></i>
                                        Tableau de bord
                                    </a>
                                    ...
                                    @if(\Illuminate\Support\Facades\Route::has('profile.edit'))
                                        <a href="{{ route('profile.edit') }}"
                                           class="px-3 py-2 rounded-xl hover:bg-gray-50 text-sm flex items-center gap-2">
                                            <i class="fa-solid fa-user-gear text-[#b58900] w-4"></i>
                                            Profil
                                        </a>
                                    @endif

                                    @if(\Illuminate\Support\Facades\Route::has('password.edit'))
                                        <a href="{{ route('password.edit') }}"
                                           class="px-3 py-2 rounded-xl hover:bg-gray-50 text-sm flex items-center gap-2">
                                            <i class="fa-solid fa-key text-[#b58900] w-4"></i>
                                            Mot de passe
                                        </a>
                                    @endif
                                    ...
                                    @if(\Illuminate\Support\Facades\Route::has('appearance.edit'))
                                        <a href="{{ route('appearance.edit') }}"
                                           class="px-3 py-2 rounded-xl hover:bg-gray-50 text-sm flex items-center gap-2">
                                            <i class="fa-solid fa-circle-half-stroke text-[#b58900] w-4"></i>
                                            Apparence
                                        </a>
                                    @endif
                                    ...
                                    @if(\Illuminate\Support\Facades\Route::has('two-factor.show'))
                                        <a href="{{ route('two-factor.show') }}"
                                           class="px-3 py-2 rounded-xl hover:bg-gray-50 text-sm flex items-center gap-2">
                                            <i class="fa-solid fa-shield-halved text-[#b58900] w-4"></i>
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit"
                                                class="w-full px-3 py-2 rounded-xl hover:bg-red-50 text-sm flex items-center gap-2 text-red-600">
            ...
            <div id="mobileMenu" class="md:hidden hidden gap-3 pb-4 pt-2">
                ...
                <nav class="grid gap-1 bg-white/90 rounded-2xl border aa-border p-2 backdrop-blur-md">
                    ...
                    @auth
                        @if(\Illuminate\Support\Facades\Route::has('client.messages'))
                            <a href="{{ route('client.messages') }}"
                               class="px-4 py-2 rounded-lg hover:bg-gray-50 flex items-center justify-between">
                                <span>Messages</span>
                                @if(($headerNonLusCount ?? 0) > 0)
                                    <span class="h-6 min-w-[24px] px-2 grid place-items-center rounded-full bg-[#facc15] text-[#1C1C1C] text-xs font-extrabold shadow">
                                        {{ $headerNonLusCount }}
                                    </span>
                                @endif
                            </a>
                        ...
                        @if(\Illuminate\Support\Facades\Route::has('favoris.index'))
                            <a href="{{ route('favoris.index') }}" class="px-4 py-2 rounded-lg hover:bg-gray-50">Favoris</a>
                        @endif

                        @if(\Illuminate\Support\Facades\Route::has('profile.edit'))
                            <a href="{{ route('profile.edit') }}" class="px-4 py-2 rounded-lg hover:bg-gray-50">Profil</a>
                        @endif

                        @if(\Illuminate\Support\Facades\Route::has('password.edit'))
                            <a href="{{ route('password.edit') }}" class="px-4 py-2 rounded-lg hover:bg-gray-50">Mot de passe</a>
                        @endif

                        @if(\Illuminate\Support\Facades\Route::has('appearance.edit'))
                            <a href="{{ route('appearance.edit') }}" class="px-4 py-2 rounded-lg hover:bg-gray-50">Apparence</a>
                        @endif

                        @if(\Illuminate\Support\Facades\Route::has('two-factor.show'))
                            <a href="{{ route('two-factor.show') }}" class="px-4 py-2 rounded-lg hover:bg-gray-50">Double authentification</a>
                        @endif
...
{{-- IMPORTANT : scripts pushés depuis les pages --}}
@stack('scripts')

{{-- JS header/menu + dropdown profil --}}
<script>
    (function(){
        ...
        if (menuBtn && mobileMenu) {
            menuBtn.addEventListener('click', () => {
                ...
                if (!mobileMenu.classList.contains('hidden')) mobileMenu.classList.add('grid');
                else mobileMenu.classList.remove('grid');
            });
        }

        // Dropdown profil (desktop)
        const ddBtn = document.getElementById('profileDropdownBtn');
        const ddMenu = document.getElementById('profileDropdownMenu');
        const ddWrap = document.getElementById('profileDropdownWrap');
        ...
        if (ddBtn && ddMenu && ddWrap) {
            ...
            document.addEventListener('click', (e) => {
                if (!ddWrap.contains(e.target)) ddMenu.classList.add('hidden');
            });
        }
    })();
</script>
